...
/* send from pcustrips() to pcuhit() every 32ns; we assumes that only one pulse can exist in every 32ns interval */
typedef struct pcustrip_s
{
...
void pcustrips(ap_uint<16> strip_threshold, hls::stream<fadc_256ch_t> &s_fadcs, PCUStrip_s &s_strip);
void pcuhit(ap_uint<16> thresholdmin, ap_uint<16> thresholdmax, nframe_t nframes, PCUStrip_s s_strip, PCUHit s_hit[NH_READS]);
