...
void
pcuhiteventreader(hls::stream<eventdata3_t> &event_stream, PCUHit_8slices &hit, uint32_t *bufout)
{
...
pculib(uint32_t *bufptr, uint16_t threshold_[3], uint16_t nframes_)
{
  ...
  for(sec=0; sec<NSECTOR; sec++)
  {
    ...
    pcuhiteventreader(event_stream, hit, bufout);
#ifdef DEBUG
    for(int i=0; i<=bufout[0]; i++) printf("PCU bufout[%d]=0x%08x\n",i,bufout[i]);
#endif
    if(bufout[0]>0) /*bufout contains something */
	{
      int fragtag = 60094+sec; /* pcu vtp rocid range is 94-99 */
      int banktag = 0xe122;
