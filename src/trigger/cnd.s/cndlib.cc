...
cndhiteventreader(hls::stream<eventdata3_t> &event_stream, CNDHit_8slices &hit, uint32_t *bufout)
{
  ...
  while(1)
  {
    ...
    {
      bufout[0] = 0;
      printf("cndhiteventreader: EMPTY STREAM ERROR1\n");
      break;
    }
...
void
cnd_buf_ram_to_event_buf_ram(hit_ram_t buf_ram[512], event_ram_t event_buf_ram[4096])
{
  for(int i=0; i<512; i++)
  {
...
void
cndlib(uint32_t *bufptr, uint16_t threshold_[3], uint16_t nframes_)
{
  int status, ret, sec;
  int iev;
