...
void
pcustrips(ap_uint<16> strip_threshold, hls::stream<fadc_256ch_t> &s_fadcs, PCUStrip strip[NSTRIP])
{
...
//#pragma HLS INTERFACE axis register both port=strip
...
#pragma HLS ARRAY_PARTITION variable=out complete dim=1
  ...
  {
    strip[i].en = energytmp[i];
    strip[i].tm = timetmp[i];
#ifdef DEBUG
    cout<<"pcustrips: strip["<<i<<"].en="<<strip[i].en<<", strip["<<i<<"].tm="<<strip[i].tm<<endl;
#endif
  }
