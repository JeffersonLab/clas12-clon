...
int dcdsectEEL(int, TCanvas *fCanvas, int, float, float, float,float);
...
void Viewer::MousePosition(Int_t event, Int_t ix, Int_t iy, TObject *selected)
{
  ...
  else if (event == kButton1Up)
  {
    ...
    dcdsectEEL(0, fCanvas, 1, sxy, x0, y0, fT0);
  }
...
Viewer::DoButton()
{
  ...
  /*Nevt = sdakeys_.ltrig[1];*/
   ...
   dcdsectEEL(first, fCanvas, 1, sxy, x0, y0, fT0);
  ...
  */
  ...
  if(batch)
  {
    ...
    for(iev=0; iev<3500; iev++)
    {
      ...
      status = dcdsectEEL(handler, 0, 1, sxy, x0, y0, fT0);
      if(status==EOF) break;
    }
    /*dcoutput();*/
  }
