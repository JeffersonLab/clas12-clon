...
void
htcchiteventwriter(hls::stream<trig_t> &trig_stream, hls::stream<eventdata3_t> &event_stream, event_ram_t buf_ram_read[2048])
{
...
//#pragma HLS ARRAY_PARTITION variable=buf_ram_read block factor=4
  ...
  while(addr != stop_addr)
  {
...
#ifdef DEBUG
	std::cout<<"htcchiteventwriter: buf_ram_val.output="<<buf_ram_val.output<<endl;
#endif
    ...
    if(buf_ram_val.output != 0)
	{
      ...
      eventdata.data[0](30,27)  = HTCCHIT_TAG;
      eventdata.data[0](26,16)  = addr - start_addr;  /*8 bits*/
      eventdata.data[0](15,0)   = /*buf_ram_val.standalone*/0;
      //event_stream.write(eventdata);
#ifdef DEBUG
	  std::cout<<"htcchiteventwriter: write1="<<eventdata.data<<endl;
#endif
      ...
      //event_stream.write(eventdata);
#ifdef DEBUG
	  std::cout<<"htcchiteventwriter: write2="<<eventdata.data<<endl;
#endif
      ...
      event_stream.write(eventdata);
#ifdef DEBUG
	  std::cout<<"htcchiteventwriter: write3="<<eventdata.data<<endl;
#endif
	}
