...
void ftClusterEventReader(hls::stream<eventdata_t> &event_stream, FTCluster_t clusters[FT_MAX_CLUSTERS * MAXTIMES], uint32_t *bufoutADCFT1, 
	...
	bool present = false;  
#ifdef DEBUG 
	printf("ftClusterEventReader called, two buffers are ==> adcft1: 0x%08x <== ==> adcft2: 0x%08x <== \n", bufoutADCFT1, bufoutADCFT2); 
	fflush(stdout); 
#endif  
	...
	while (1) { 
		...
		*(*bufptr) = eventdata.data; /*Write to output*/ 
		...
		if (eventdata.end == 1) { /*Write number of words in first word FOR BOTH BUFFERS!!!*/ 
			bufoutADCFT1[0] = bufptr1 - bufoutADCFT1 - 1; 
			bufoutADCFT2[0] = bufptr2 - bufoutADCFT2 - 1; 
#ifdef DEBUG 
			printf("ftClusterEventReader: END_OF_DATA, ROC is: %i \n", (int) ROC + 1); 
			fflush(stdout); 
