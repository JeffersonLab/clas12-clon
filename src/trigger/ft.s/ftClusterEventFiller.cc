...
void
ftClusterEventFiller(hls::stream<FTCluster_t> &s_clusterin, /*cluster_ram_t buf_ram[FT_MAX_CLUSTERS][256],ap_uint<8> nClusters[256]*/hls::stream<FTCluster_t> &s_clusterout)
{
	...
	while (!s_clusterin.empty()){
        ...
        /* stream out instead of writing to ram, since it is not for real fpga */
        s_clusterout.write(fifo);
		/*
