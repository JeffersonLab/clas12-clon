...
void
ctofhiteventwriter(hls::stream<trig_t> &trig_stream, hls::stream<eventdata3_t> &event_stream, event_ram_t buf_ram_read[2048])
{
...
//#pragma HLS ARRAY_PARTITION variable=buf_ram_read block factor=4
  ...
  while(addr != stop_addr)
  {
...
#ifdef DEBUG
	std::cout<<"ctofhiteventwriter: buf_ram_val.output="<<buf_ram_val.output<<endl;
#endif
    ...
    if(buf_ram_val.output != 0)
	{
      ...
      eventdata.data[0](15,0)   = buf_ram_val.standalone;
      //event_stream.write(eventdata);
#ifdef DEBUG
	  std::cout<<"ctofhiteventwriter: write1="<<eventdata.data<<endl;
#endif
      ...
      event_stream.write(eventdata);
#ifdef DEBUG
	  std::cout<<"ctofhiteventwriter: write3="<<eventdata.data<<endl;
#endif
	}
