...
public:
    ...
    IpcConsumer(const std::string& brokerURI, int numMessages, bool useTopic = true, bool sessionTransacted = false, int waitMillis = 30000) :
        latch(1),
    ...
    virtual void run() {
        ...
        try {
            ...
            if (useTopic) {
	      //	  destination = session->createTopic("TEST.FOO"/*"HEART-BEAT-CHANNEL"*//*"*"*/);
			  destination = session->createTopic("KEN");
            } else {
	      //		  destination = session->createQueue("TEST.FOO"/*"HEART-BEAT-CHANNEL"*//*"*"*/);
	      destination = session->createQueue("KEN");
            }
