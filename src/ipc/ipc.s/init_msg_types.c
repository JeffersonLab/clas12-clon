...
init_msg_types(void)
{
  int i;
  T_IPC_MT mt;
  IPC_MSG_TYPE_LOCAL(2);

  printf("\nCalls init_msg_types()\n\n");
  /* create clas standard message types and register in status or data catagory */
  for(i=0; i<sizeof(ipc_msg_type2)/sizeof(struct clas_standard_msg_type); i++)
  {
    mt=TipcMtCreate(ipc_msg_type2[i].mtname,ipc_msg_type2[i].mtnum,"verbose");
