...
void construct_output(output *out)
{
  ...
  while (onode = out->current())
  {
	...
	{
	  msg << icomp->timestamp() /*sergey << Check*/;
	  break;
	}
	inputlist.next();
      }
...
//	    msg << n->value << Check;
	  ...
	  case t_string:	// a character string
	  {
	    ...
	    msg << n->str() /*sergey << Check*/;
	    break;
	  }
	  case t_tagged:
	    msg << inode->name() /*sergey << Check*/;
	...
	else if (inode = icomp->find(oc->name()))
	{
	  ...
	  case t_array:		// an array of numbers.
	  {
	    ...
	    if (n->arr_type() == arr_int)
	    {
	      ...
	      arr[0] = 0;
	      DEBUG(cout << "integer" << endl);
	      msg << SetSize(1) << (T_INT4 *)arr /*sergey << Check*/;
	    }
	    else if (n->arr_type() == arr_float)
	    {
	      DEBUG(cout << "float" << endl);
	      ...
	      msg << SetSize(1) << (T_REAL8 *)arr /*sergey << Check*/;
	    }
	    break;		// breaks the inner switch.
	  }
	  }
...
{
  TipcMsgFile msgfile(const_cast<char *>(filename), T_IPC_MSG_FILE_CREATE_WRITE);
  if (!msgfile)
  {
...
{
  TipcMsgFile msgfile(const_cast<char *>(filename),T_IPC_MSG_FILE_CREATE_READ);
  if (!msgfile)
  {
