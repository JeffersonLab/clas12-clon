...
//sergey: in info_classes: friend T_IPCX_API TipcMsg& Check(TipcMsg& msg);
...
public:
  ...
  void MsgWrite(TipcMsg &m)
  {
    if (a_type == arr_int)
    {
      ...
      {
      case int2:
        m << SetSize(a_length) << a_data.ints2 << Check(m);
        break;
      case int4:
        m << SetSize(a_length) << a_data.ints4 << Check(m);
        break;
      case int8:
        m << SetSize(a_length) << a_data.ints8 << Check(m);
        break;
      }
    }
    else if (a_type == arr_float)
    {
      ...
      {
      case flt4:
        m << SetSize(a_length) <<  a_data.floats4 << Check(m);
        break;
      case flt8:
        m << SetSize(a_length) <<  a_data.floats8 << Check(m);
        break;
      case flt16:
        m << SetSize(a_length) <<  a_data.floats16 << Check(m);
        break;
      }
    }
  }
  ...
  void MsgRead(TipcMsg &m, T_IPC_FT ft = T_IPC_FT_INVALID)
  {
    ...
    case T_IPC_FT_REAL16_ARRAY:
      m >> r16 >>  GetSize(&a_length) >> Check(m);
      myfsize = flt16;
      a_type = arr_float;
