...
// for gamma data
static int    ngamma = sizeof(epics)/sizeof(epics_struct /*sergey: was 'char*' */);
...
channel_thread(void *param)
{
  ...
  if(no_ipc==0)
  {
    /*printf("channel_thread: nepics=%d\n",nepics);*/

    nev_to_ipc++;
    for(int i=0; i<nepics; i++) bosarray[i][0] = epics_val[i];

	server << clrm << "evio2et" << "EPIC" << (int32_t)0 << "(F,8A)" << (int32_t)9 << (int32_t)nepics 
		   << (int32_t)(nepics*9) << SetSize(nepics*9) << (int32_t*)bosarray << endm;
...
gamma_thread(void *param)
{
  ...
  if(no_ipc==0)
  {
    /*printf("gamma_thread: ngamma=%d\n",ngamma);*/

    nev_to_ipc++;
    for(int i=0; i<ngamma; i++) gammaarray[i][0] = gamma_val[i];

	server << clrm << "evio2et" << "EPIC" << (int32_t)1 << "(F,8A)" << (int32_t)9 << (int32_t)ngamma
	       << (int32_t)(ngamma*9) << SetSize(ngamma*9) << (int32_t*)gammaarray << endm;
                   ...
                   size_t nscaler, float *scalers, const char* channel)
{
  ...
  if(nscaler>0)
  {
    ...
    if(no_ipc==0)
    {
      printf("unpack_scaler_data 1\n");

      nev_to_ipc++;

	  server << clrm << "evio2et" << (char *)bank << (int32_t)nrecord << "(F)" << (int32_t)1 
